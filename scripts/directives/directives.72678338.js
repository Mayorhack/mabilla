...
mifosX.ng.application
  .directive("ngDisplaypanel", [
    mifosX.directives.ActivitiesDisplayPanelDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("ngAutofocus", [
    "$timeout",
    "$parse",
    mifosX.directives.AutofocusDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("panelbig", [mifosX.directives.BigPanelDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("chosen", ["$compile", mifosX.directives.ChosenComboboxDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("ckEditor", [mifosX.directives.CkEditorDirective])
  .run(function ($log) {
    ...
    DecimalDirective: function ($compile) {
      var linker = function (scope, element, attrs, ngModel) {
        ...
        ngModel.$parsers.unshift(function (newValue) {
          ...
          if (isNaN(parseFloat(newValue))) element.val(null);
...
mifosX.ng.application
  .directive("decimals", ["$compile", mifosX.directives.DecimalDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("mfDialog", [mifosX.directives.dialogDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("rcSubmit", [
    "$parse",
    mifosX.directives.FormSubmitValidateDirective,
  ])
  .run(function ($log) {
      ...
      return {
        ...
        link: function (scope, elm, attr, ctrl) {
          ...
          var template =
            ...
            " | translate}}" +
            "</small>" +
            '<small class="required error" ng-show="' +
            scope.formNameAttribute +
            "." +
            scope.inputAttributeName +
            '.$error.nval">' +
...
mifosX.ng.application
  .directive("formValidate", [
    "$compile",
    mifosX.directives.FormValidateDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("hasPermission", [
    "$rootScope",
    mifosX.directives.HasPermissionDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("lateValidate", [mifosX.directives.LateValidateDirective])
  .run(function ($log) {
        ...
        link: function (scope, element, attrs, modelCtrl) {
          ...
          function initialNumber(value) {
            ...
            if (
              ...
              stringValue.indexOf(decimalSep) > 0 &&
            ...
            if (
              stringValue != undefined &&
              stringValue.indexOf(DECIMAL_SEP) > 0 &&
              decimalSep != DECIMAL_SEP
            ) {
...
mifosX.ng.application
  .directive("numberFormat", [
    ...
    "$locale",
    "$parse",
    mifosX.directives.NumberFormatDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("ngOnBlur", ["$parse", mifosX.directives.OnBlurDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("mfOverlay", [mifosX.directives.overlayDirective])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("panel", [mifosX.directives.PanelDirective])
  .run(function ($log) {
        ...
        link: function ($scope, wrappedElement) {
          ...
          function onDocumentMouseUp($event) {
            ...
            if (dragInfo.moved === true) {
              [].forEach.call(
                navTabsElement.querySelectorAll("li a"),
                function (anchor) {
          ...
          function checkForScroll() {
            ...
            if (currentlyScrollable === true) {
              difference =
                navTabsElement.scrollWidth - navTabsElement.clientWidth;
            } else {
              difference =
                navTabsElement.clientHeight -
                navTabsElement.querySelector(".nav-tabs > li").clientHeight;
            }
          ...
          function bindEventListeners() {
            ...
            $scope.$on("$destroy", function () {
              ...
              angular
                .element(navTabsElement)
                .off("mousedown", onNavTabsMouseDown);
...
mifosX.ng.application
  .directive("scrollable", [
    ...
    "$timeout",
    "$window",
    mifosX.directives.ScrollableDirective,
  ])
  .run(function ($log) {
      ...
      return {
        link: function (scope, element, attrs) {
          ...
          function widthListener() {
            if (anchor.width() != lastWidth) updateScroll();
          }
          ...
          element.prepend(
            "<br/><div class='wrapper1'><div class='div1'></div></div>"
          );
...
mifosX.ng.application
  .directive("scroll", [mifosX.directives.ScrollbarTopDirective])
  .run(function ($log) {
          ...
          scope.$watch(
            ...
            function (successfulResponses) {
              ...
              if (scope.responses.length > 0) {
                ...
                for (var i = 0; i < scope.responses.length; i++) {
                  for (var j = 0; j < scope.br.length; j++) {
                    if (scope.responses[i].requestId == scope.br[j].requestId) {
                      scope.uniqueId.push(
                        JSON.parse(scope.br[j].body)[scope.identifier]
                      );
                    }
                  }
                }
                ...
                var template =
                  '<div class="success" ng-show="successfulResponses.length < batchRequests.length">' +
                  "<h4>Responses with listed <strong>" +
                  scope.identifier +
                  "s</strong> were successful</h4>" +
...
mifosX.ng.application
  .directive("successfulResponses", [
    "$compile",
    "$rootScope",
    mifosX.directives.SuccessfulResponsesDirective,
  ])
  .run(function ($log) {
...
mifosX.ng.application
  .directive("ngSummary", [mifosX.directives.SummaryDirective])
  .run(function ($log) {
  ...
  mifosX.directives = _.extend(module, {
    TabsPersistenceDirective: function (localStorageService) {
      function setSavedTab(tabsetName, tabHeading) {
        var savedTabs =
          localStorageService.getFromLocalStorage("tabPersistence");
        if (savedTabs == null) {
          savedTabs = {};
        }
...
mifosX.ng.application
  .directive("persistentTab", [
    "localStorageService",
    mifosX.directives.TabsPersistenceDirective,
  ])
  .run(function ($log) {
        ...
        link: function (scope, element, attrs) {
          ...
          if (treeId === "holidaytreeview") {
            template =
              ...
              ".setCollapsedRoot(" +
              treeModel +
              ", false)\">{{'label.button.expand.all' | translate}}</a>" +
              " / " +
              '<a data-ng-click="' +
              treeId +
              ".setCollapsedRoot(" +
              treeModel +
              ", true)\">{{'label.button.collapse.all' | translate}}</a>" +
              "</div>" +
              ...
              '<i class="expanded" data-ng-show="node.' +
              nodeChildren +
              '.length && !node.collapsed" data-ng-click="' +
              treeId +
              '.selectNodeHead(node)"></i>' +
              ...
              '<span ng-show="node.' +
              nodeId +
              ' >= 0" data-ng-class="node.selected" data-ng-click="' +
              treeId +
              ".selectNodeLabel(node); $root.tempNodeID = node." +
              nodeId +
              '">({{node.' +
              nodeglCode +
              "}}) {{node." +
          ...
          if (treeId && treeModel) {
            if (attrs.angularTreeview) {
              ...
              scope[treeId].setCollapsedAll =
                scope[treeId].setCollapsedAll ||
                function (selectedNode, state) {
                  selectedNode.collapsed = state;
                  for (var i = 0; i < selectedNode[nodeChildren].length; i++) {
                    if (
                      selectedNode[nodeChildren][i][nodeChildren].length > 0
                    ) {
                      scope[treeId].setCollapsedAll(
                        selectedNode[nodeChildren][i],
                        state
                      );
                    }
                  }
                };
              scope[treeId].setCollapsedRoot =
                scope[treeId].setCollapsedRoot ||
                function (treeModel, state) {
              ...
              scope[treeId].showChangeStateAll =
                scope[treeId].showChangeStateAll ||
                function (treeModel) {
                  if (!treeModel) {
                    return false;
                  ...
                  for (var i = 0; i < treeModel.length; i++) {
                    if (
                      treeModel[i][nodeChildren].length > 0 &&
...
mifosX.ng.application
  .directive("treeModel", ["$compile", mifosX.directives.TreeviewDirective])
  .run(function ($log) {
